...
This layered architecture enables clear separation of concerns while maintaining flexibility for future enhancements. Each layer can be modified independently without affecting other components, provided that the interfaces between layers remain stable. The architectural approach aligns with the RAGOps framework articulated by \textcite{xuRAGOpsOperatingManaging2025}, which emphasizes that RAG systems consist of multiple interdependent components requiring careful lifecycle management across both query processing and data management dimensions. Their analysis demonstrates that effective RAG system operation necessitates independent testing, monitoring, and evolution of individual components—such as embedding generation, retrieval mechanisms, and generation logic—while maintaining observability across the entire pipeline. The layered design directly supports these operational requirements by establishing clear component boundaries that facilitate isolated testing and updates, enable targeted performance monitoring at each layer, and provide explicit interfaces for observability instrumentation. This modularity proves essential for managing the continuous evolution characteristic of production RAG systems, where data sources change frequently, model providers may be swapped, and retrieval strategies require iterative refinement based on evaluation feedback \parencite{xuRAGOpsOperatingManaging2025}. Figure~\ref{fig:system-layers} illustrates the complete system architecture showing how these layers interact with each other.
...
The data persistence layer leverages PostgreSQL's pgvector extension \cite{PgvectorPgvector2025} to enable efficient similarity search over vector embeddings. This choice aligns with Sovelia Core's existing PostgreSQL infrastructure, avoiding the need for separate vector database systems as employed in some of the reviewed case studies \parencite{herediaalvaroAdvancedRetrievalaugmentedGeneration2025, shejutiExtendedAbstractEnhancing2025}. While dedicated vector database management systems offer specialized optimizations for high-dimensional data, extensions like pgvector that integrate vector capabilities within relational database management systems present significant practical advantages for enterprise deployments \parencite{taipalusVectorDatabaseManagement2024}. As \textcite{taipalusVectorDatabaseManagement2024} notes, database management systems facilitate data management through ready-made features such as transaction and access control, automated database scalability, and query optimization, freeing development resources for business-critical tasks. The unified database approach eliminates the operational complexity of maintaining separate systems for structured and vector data, reducing both infrastructure overhead and data synchronization challenges. For on-premises PLM environments where IT resources are constrained and system integration costs are significant, this architectural simplicity provides substantial benefits. Furthermore, pgvector enables efficient similarity search through specialized indexing techniques while preserving the mature capabilities that PostgreSQL provides: ACID transaction guarantees, comprehensive security mechanisms, robust backup procedures, and proven replication strategies. The schema design prioritizes both performance and data integrity through careful normalization and indexing strategies.

The core data model consists of three primary tables. The first table maintains configuration parameters for the system, storing key-value pairs that control various aspects of AI functionality including update schedules, system behavior with model and provider designation. The second table stores source document metadata, tracking documentation from three distinct source types: customer specific internal PLM documents, external vendor documentation, and web-based knowledge articles. Each source record maintains information about document identifiers, versions, titles, and URLs where applicable. The third table contains the actual content chunks with their corresponding vector embeddings, maintaining references back to their source documents through foreign key constraints.
...
For vector similarity search, an HNSW (Hierarchical Navigable Small World) \cite{malkovEfficientRobustApproximate2016} index operates on the embedding column using cosine distance as the similarity metric. This index type provides efficient approximate nearest neighbor search through a hierarchical graph structure that balances query performance with high recall quality, as recommended in the pgvector documentation for production workloads requiring both speed and accuracy. \parencite{PgvectorPgvector2025} The choice of cosine distance aligns with the embedding models used, which produce normalized vectors where cosine similarity effectively measures semantic relatedness. This distance metric choice is consistent with implementations observed in the reviewed manufacturing RAG systems \parencite{knollmeyerDocumentGraphRAGKnowledge2025}, where cosine similarity proved effective for technical documentation retrieval.
...
A critical requirement for the RAG system was maintaining Sovelia Core's existing access control model. Documents in the PLM system have granular permissions that determine which users can view specific content. The RAG system must respect these permissions to prevent information leakage through AI responses. This security consideration reflects broader concerns about data governance and access control in enterprise AI systems identified by \textcite{wangArtificialIntelligenceProduct2021}, who emphasized that security and trust remain critical challenges for AI adoption in manufacturing contexts. Additionally, regulatory frameworks such as the EU AI Act emphasize the importance of transparent access control mechanisms in AI systems \parencite{sillbergEUAIAct2025}, making robust permission filtering not only a technical requirement but also a regulatory compliance necessity for organizations operating within the European Union.

The challenge of implementing access control in RAG systems has been extensively studied in recent research. \textcite{jeongPermissionAwareRAGIdentity2025} demonstrated that most existing RAG research implicitly assumes all retrieved content is equally accessible to any user, failing to address the complex, fine-grained access control required in enterprise environments where data originates from multiple, independently governed sources. Their permission-aware RAG framework enforces resource-level access control by directly interfacing with provider-controlled Identity and Access Management (IAM) systems, performing real-time permission validation against native IAM endpoints without requiring policy merging. This approach preserves governance boundaries and ensures compliance across heterogeneous authorization models such as Role-Based Access Control (RBAC) and Attribute-Based Access Control (ABAC). Similarly, \textcite{zhongHoneyBeeEfficientRolebased2025} addressed the performance trade-offs inherent in access-controlled vector databases, noting that existing approaches face significant challenges: creating dedicated indexes for each user minimizes query latency but introduces excessive storage redundancy, while building a single index and applying access control after vector search reduces storage overhead but suffers from poor recall and increased query latency. Their HoneyBee framework introduces dynamic partitioning that leverages RBAC policy structure to balance storage efficiency with query performance.

The implementation addresses this through a permissions check embedded in the vector search function. When retrieving relevant chunks for a query, the search function accepts the requesting user's identifier and filters results based on permission criteria stored in a separate table. This criteria table defines search patterns that determine which documents should be accessible through the AI system. This database-level filtering approach aligns with the design principle identified by \textcite{jeongPermissionAwareRAGIdentity2025}, where access control enforcement should occur during retrieval rather than after generation, preventing unauthorized information from influencing the language model's output. Their evaluation demonstrated that permission-aware filtering preserves both security guarantees and answer quality, confirming that access control can be enforced without degrading RAG system effectiveness.

For internal PLM documents, each source record links back to the original document key in the PLM database. The search function performs a permissions check against this original document before including chunks in results. External documentation sources bypass this check since they are publicly available in the internet as official vendor documentation. This dual approach mirrors the multi-resource environment discussed by \textcite{jeongPermissionAwareRAGIdentity2025}, where different data sources have different governance models and access policies. Their framework demonstrates that maintaining separate permission validation mechanisms for different resource types preserves governance boundaries while ensuring compliance with diverse access policies.

This approach ensures that the AI system acts as an extension of the existing PLM interface rather than a separate system with its own permission model. Users can only retrieve information through the AI that they could access through traditional navigation and search. The design addresses the fundamental security requirement that RAG systems must not become backdoor channels for accessing restricted information, a concern emphasized by both \textcite{jeongPermissionAwareRAGIdentity2025} and \textcite{zhongHoneyBeeEfficientRolebased2025}. While the current implementation uses search criteria-based filtering rather than direct IAM integration, the architectural foundation supports future enhancement with more sophisticated permission validation mechanisms as enterprise requirements evolve.
...
Vendor lock-in represents a critical risk in cloud-based AI systems, particularly as organizations migrate computing resources from on-premise to external services. \textcite{opara-martinsCriticalAnalysisVendor2016} demonstrated that proprietary standards inhibit interoperability and portability, with lock-in problems becoming exacerbated during cloud migration due to lack of standardization. Their analysis identified key risk factors including API dependencies, data portability constraints, and contract limitations that impose switching costs. For RAG systems specifically, these concerns manifest in provider-specific embedding formats that require complete re-vectorization when switching models, proprietary API structures that couple application logic to vendor implementations, and varying rate limiting approaches that complicate operational predictability. Furthermore, \textcite{wangLargeLanguageModel2025} analyzed the multi-layered dependencies in LLM supply chains, demonstrating that systems spanning infrastructure, foundation models, and downstream applications face cascading vulnerabilities when tightly coupled to single providers. Their software engineering perspective emphasized that model versioning inconsistencies, evolving provider SDKs, and heterogeneous security models introduce complexity that abstraction layers effectively isolate from core application logic. For PLM environments with 7-15 year system lifecycles \cite{shilovitskyTrajectoriesOldPLM2021}, these risks prove particularly acute given the rapid evolution of LLM provider landscapes measured in months rather than years.

The abstraction layer supports four provider types: OpenAI's cloud-based API \cite{APIPlatformOpenAI}, Azure OpenAI service for organizations requiring Microsoft's infrastructure \cite{mrbullwinkleWhatAzureOpenAI}, Groq for high-performance inference \cite{GroqFastLow2025}, and Ollama \cite{Ollama} for fully on-premises deployment. Each provider requires different configuration parameters and authentication methods, but the abstraction layer presents a uniform interface to the rest of the system. This multi-provider support directly addresses the on-premises deployment constraint emphasized in the research questions while maintaining flexibility for organizations with different security postures. The design implements mitigation strategies identified by \textcite{opara-martinsCriticalAnalysisVendor2016}, including standardized configuration formats, explicit separation of provider-specific dependencies, and vendor-neutral interfaces that preserve portability across different deployment models.
...
Each chunk produces a 768-dimensional vector representing its semantic content. The dimensionality matches Nomic's nomic-embed-text model \parencite{nussbaumNomicEmbedTraining2025}, which was selected for its balance of quality and efficiency. Vector embeddings represent a fundamental technique for capturing semantic relationships in low-dimensional spaces, enabling efficient similarity computation over large document collections \parencite{chenTutorialNetworkEmbeddings2018}. While higher-dimensional embeddings might capture more nuanced semantic relationships, they also increase storage requirements and query latency. This embedding model choice reflects the pragmatic approach emphasized in the literature review (\autoref{sec:case-studies-on-similar-integrations}), prioritizing production-ready models with established performance characteristics over experimental alternatives requiring extensive validation.
...
Before inserting chunks for a document, the system deletes any existing chunks associated with that document's source identifier. This approach ensures that document updates properly replace old content rather than accumulating multiple versions. The deletion operation leverages the cascade behavior defined in the foreign key constraints, automatically removing all chunk records when their parent source record is deleted. This delete-then-insert pattern aligns with data management best practices articulated in the RAGOps framework \parencite{xuRAGOpsOperatingManaging2025}, which emphasizes that the updating retrieval sources phase must handle modifications through proper removal of outdated information to prevent retrieval of stale or contradictory content. Their analysis of the data management lifecycle identifies version replacement as essential for maintaining retrieval quality, as accumulating multiple versions of the same document introduces semantic redundancy that degrades retrieval precision and may cause the system to present conflicting information to users.

After deletion, the system inserts a new source record (or updates the existing one if it already exists) followed by insertion of all chunk records. This two-phase operation maintains referential integrity while enabling atomic document updates. The approach implements the data verification principles outlined by \textcite{xuRAGOpsOperatingManaging2025}, particularly regarding recency checks where older versions should be replaced rather than retained alongside current content. Their framework emphasizes that effective data management requires systematic elimination of outdated records to maintain consistency, as retrieving from multiple versions of the same document can lead to temporal confusion where the LLM blends information from different document states.

For internal PLM documents, the deletion logic includes special handling to remove all versions of a document when any version is updated. PLM documents have a document identifier that remains constant across versions, while each version receives a unique document key. When updating a document, the system looks up the document identifier from the provided key and deletes all source records associated with that identifier, making sure that old versions don't appear in search results alongside the current version. This version management strategy directly addresses the recency verification requirement identified by \textcite{xuRAGOpsOperatingManaging2025}, where data verification must ensure that older versions are archived and replaced by more recent versions rather than coexisting in the retrieval source. Their operational guidance emphasizes that version control mechanisms must prevent retrieval systems from accessing superseded content, as stale information undermines the accuracy guarantees that RAG systems provide over purely parametric LLM knowledge.
...
As discussed in Section~\ref{subsec:access-control-integration}, the retrieval function integrates permission checks directly in the database query. This database-level filtering provides better performance than application-level filtering because it avoids retrieving chunks that will subsequently be discarded. This architectural choice reflects the performance considerations documented by \textcite{zhongHoneyBeeEfficientRolebased2025}, who demonstrated that access control strategies in vector databases face fundamental trade-offs between storage redundancy, query latency, and recall quality. Their analysis showed that applying access control filters during vector search (hybrid queries) rather than after retrieval maintains both security and performance, avoiding the recall degradation that occurs when post-processing filters remove relevant results.

The permission check operates through a search criteria that defines patterns for matching documents. The search itself is handled by custom search engine inside Sovelia Core. Each criterion consists of search conditions that can match documents based on various attributes like document type, status, or custom fields. When a document is created or modified and then saved to the system, it evaluates all criteria to determine if the document should be vectorized and made available through the AI system. This criteria-based approach provides flexibility similar to attribute-based access control (ABAC) mechanisms, where access decisions depend on document properties rather than only user roles, though the implementation remains simpler than full ABAC systems \parencite{jeongPermissionAwareRAGIdentity2025}.

During retrieval, the search function evaluates these same criteria against internal documents before including their chunks in results. This ensures consistency between which documents are vectorized and which documents appear in search results. As mentioned in Section~\ref{subsec:access-control-integration}, external documentation sources bypass this check, as they represent vendor-provided documentation that should be available to all users. This differentiated treatment of internal versus external content aligns with the multi-resource permission model described by \textcite{jeongPermissionAwareRAGIdentity2025}, where heterogeneous data sources require different permission validation strategies to maintain their respective governance boundaries.

This design allows organizations to implement custom access policies easily through configuration UI. For example, an organization might restrict AI access to only released documents, or might limit access to documents in specific product lines based on user roles. While the current implementation operates at the document level rather than implementing fine-grained vector-level permissions, this design choice reflects the practical trade-offs identified by \textcite{zhongHoneyBeeEfficientRolebased2025}: document-level filtering provides sufficient security for most PLM use cases while avoiding the storage overhead and complexity of vector-level partitioning schemes. Future enhancements could incorporate dynamic partitioning strategies if more granular access control becomes necessary, particularly for organizations with complex RBAC hierarchies spanning multiple business units or product lines.
...
The system employs a structured prompt template that combines system instructions, conversation history, retrieved context, and the user's question. This structure follows best practices for RAG implementations as documented in the LangChain framework \cite{LangChainOverview} and aligns with successful patterns observed in the reviewed case studies, while being adapted to the specific domain of PLM systems. The approach implements in-context retrieval-augmented generation, where retrieved documents are prepended to the input without modifying the underlying language model architecture \parencite{ramInContextRetrievalAugmentedLanguage2023}. This architectural simplicity facilitates deployment across different LLM providers while maintaining the core benefits of grounding generation in retrieved documentation.

The system prompt establishes the AI assistant's role and behavior. It identifies the assistant as an integrated component of the PLM system, setting expectations for the types of questions it can answer. The prompt includes explicit instructions about citation behavior, instructing the model to reference provided URLs when available and to avoid fabricating links or information not present in the retrieved context. This explicit anti-hallucination instruction addresses a key risk factor identified in RAG system deployments \parencite{lewisRetrievalAugmentedGenerationKnowledgeIntensive2021}, where models may blend parametric knowledge with retrieved context in ways that reduce factual accuracy. Furthermore, transparency requirements emphasized by regulatory frameworks such as the EU AI Act \parencite{sillbergEUAIAct2025} necessitate clear provenance tracking and source attribution in AI-generated responses, making citation mechanisms not merely a quality feature but a compliance requirement for enterprise deployments in regulated environments.
...
This flexibility acknowledges that different organizations face different constraints. For example, a large enterprise with strict data governance requirements might mandate on-premises deployment despite performance trade-offs, while a smaller organization might prioritize response quality and operational simplicity through cloud deployment. The synergy between retrieval and generation components proves crucial for effective RAG performance, with iterative retrieval-generation interactions helping to identify correct reasoning paths particularly for tasks requiring multi-step reasoning \parencite{fengRetrievalGenerationSynergyAugmented2024}. While the current implementation employs a single-stage retrieval approach, the architectural foundation supports future enhancement with iterative retrieval strategies as system maturity and user requirements evolve.
...
The API implements comprehensive error handling to provide clear feedback when operations fail. Errors are categorized by type (validation errors, permission errors, service errors) and include descriptive messages suitable for display to end users. This structured approach to error handling addresses key challenges identified by \textcite{coblenzQualitativeStudyREST2023} in their qualitative study of REST API design practices, where participants reported that standard HTTP error codes proved insufficient for conveying detailed information to clients and end users, and that inconsistent error reporting methods complicated debugging.

Validation errors occur when request payloads violate constraints like maximum prompt length or missing required fields. These return HTTP 400 status codes with messages explaining the validation failure. The implementation follows the recommendation by \textcite{coblenzQualitativeStudyREST2023} to use standardized JSON structures for error responses, where each error includes both the HTTP status code and a descriptive message. This dual representation enables programmatic error handling by client applications while providing human-readable feedback when errors are displayed to users. The approach addresses a fundamental challenge in REST API design: insuficient documentation of input requirements frequently leads to bugs when clients send malformed data \parencite{coblenzQualitativeStudyREST2023}, making explicit validation with clear error messages essential for robust client-server interactions.
...
Service errors occur when dependencies like the database or embedding service encounter failures. These return HTTP 500 status codes and log detailed error information for investigation while providing generic error messages to users to avoid information disclosure. This separation of detailed internal logging from user-facing messages reflects best practices identified by \textcite{coblenzQualitativeStudyREST2023}, where participants noted that overly detailed error messages can leak sensitive information about server implementations, creating security vulnerabilities. The approach balances the competing needs of developers requiring detailed diagnostic information with the security requirement to prevent information disclosure to potentially malicious actors.
...
Research by \textcite{budiuTypesConversationsGenerative2023} demonstrated through analysis of 425 AI interactions that users engage in six distinct conversation types with generative AI systems, ranging from single-prompt search queries to multi-turn exploratory dialogues. Their findings revealed no correlation between conversation length and perceived helpfulness or trustworthiness, indicating that effective AI interfaces must support both concise exchanges for well-defined information needs and extended dialogues for exploratory learning. This diversity of interaction patterns informed the decision to implement a flexible chat interface capable of accommodating different user goals without imposing unnecessary constraints on conversation structure.

Figure~\ref{fig:sovelia-chat-ui} shows the chat interface integrated into Sovelia Core's web client. The interface appears as a popup that can be opened or closed without disrupting the main PLM workflow.
...
Messages alternate between user queries and AI responses, with visual styling distinguishing between the two. AI responses support Markdown rendering, allowing formatted text with emphasis, lists, and inline code when appropriate. This formatting capability enables more readable responses for technical content, particularly important given that PLM documentation queries often span multiple conversation types identified by \textcite{budiuTypesConversationsGenerative2023}: from pinpointing conversations seeking specific procedural details to exploring conversations investigating product configurations in depth.

Source references appear alongside AI responses as interactive elements. For internal documents, clicking a source reference opens the document object in Sovelia Core. For external documentation, clicking a source reference opens the URL in a new browser tab. This integration enables seamless transition from AI-assisted discovery to detailed document review. Showing the sources directly in the UI addresses the transparency and verifiability requirements identified by \textcite{lewisRetrievalAugmentedGenerationKnowledgeIntensive2021}, where users must be able to trace AI-generated information back to authoritative documents to build trust and ensure accuracy in technical contexts. The prominence of source references also supports exploring conversations, as \textcite{budiuTypesConversationsGenerative2023} observed that users engaged in depth-focused learning benefit from explicit pathways to detailed documentation that can inform subsequent followup questions.
...
The per-message feedback granularity proves particularly valuable given the diversity of conversation types users engage in with AI assistants. Research by \textcite{budiuTypesConversationsGenerative2023} revealed that different conversation types serve distinct information needs: funneling conversations refine initially vague queries, pinpointing conversations deliver specific outputs from detailed prompts, exploring conversations pursue depth of understanding, and chiseling conversations acquire breadth across multiple facets of a topic. Message-level feedback enables identification of which conversation types and response characteristics correlate with user satisfaction, supporting targeted improvements to prompts, retrieval strategies, or result presentation. For instance, negative feedback on responses within exploring conversations might indicate insufficient depth or missing pathways to related concepts, while negative feedback in pinpointing conversations could signal misalignment between user specifications and generated output.
...
Users can clear conversation history to start fresh conversations. This proves useful when switching topics or when the conversation context becomes unwieldy. The clear action affects only the current conversation context, preserving conversations in other contexts. This functionality acknowledges that PLM users may transition between different conversation types as their information needs evolve. \textcite{budiuTypesConversationsGenerative2023} observed that users sometimes begin conversations as one type before morphing into another; for example, chiseling conversations exploring multiple facets of a component may incorporate exploratory elements that delve into specific technical details. The ability to clear conversation history enables users to reset context when shifting between fundamentally different information-seeking modes, such as moving from exploring conversations about system architecture to pinpointing conversations generating specific configuration templates.

Deleted conversations remain in the database with a deletion flag rather than being physically removed. This soft deletion approach supports potential future features like conversation restoration or administrative audit trails while preventing accidental permanent data loss. Additionally feedback associated with deleted conversations remains available for analysis, preserving valuable data about response quality. Retaining conversation metadata enables retrospective analysis of conversation type distributions, length patterns, and user satisfaction across different query categories, supporting the continuous refinement emphasized by \textcite{budiuTypesConversationsGenerative2023} where understanding conversation structures informs interface improvements tailored to specific information needs.

\section{Operational considerations}
\label{sec:operational-considerations}

Beyond core functionality, the implementation includes several features supporting operational deployment and maintenance in production environments. These operational design choices reflect established practices for managing RAG systems in production, including logging, monitoring, configuration management, and resource allocation strategies \parencite{xuRAGOpsOperatingManaging2025}.

\subsection{Logging and monitoring}
\label{subsec:logging-monitoring}

The embedding service maintains detailed logs of its processing activities. These logs capture when documents are vectorized, how many chunks are created, processing duration, and any errors encountered. Log files rotate daily with retention policies to prevent unbounded disk usage. This logging approach aligns with the observability infrastructure layer recommended by \textcite{xuRAGOpsOperatingManaging2025}, who emphasize that comprehensive logging across all RAG components provides essential visibility into system operations, enabling effective monitoring and continuous evaluation.

Application server logs capture API requests and responses, including query processing time and error conditions. These logs enable monitoring of system usage patterns and performance characteristics. The implementation addresses multiple quality attributes identified in the RAGOps framework \parencite{xuRAGOpsOperatingManaging2025}: monitorability through clear performance metrics (latency, relevance), observability through comprehensive operational visibility, and traceability through detailed logging of inputs and outputs at each processing stage.

The logging implementation uses structured log messages with consistent formatting to enable automated parsing and analysis. Key events include unique correlation identifiers that can be traced across system components, facilitating investigation of issues involving multiple services. This structured logging approach directly supports the traceability requirements articulated by \textcite{xuRAGOpsOperatingManaging2025}, where maintaining detailed records of retrieval routing, grounded sources, and component interactions enables debugging and ensures system accountability over time.

\subsection{Configuration management}
\label{subsec:configuration-management}

System behavior is controlled through a hybrid configuration approach combining database-stored settings with environment variables. Runtime-configurable parameters are stored in a dedicated \texttt{AIConfig} database table, enabling dynamic updates without application restarts. More static deployment parameters, such as database connection strings and server-specific settings, remain as environment variables following twelve-factor application principles \parencite{TwelveFactorApp}. This configuration strategy reflects the adaptability quality attribute emphasized by \textcite{xuRAGOpsOperatingManaging2025}, where automated updates to retrieval sources and system components must be supported to maintain system effectiveness as requirements evolve.

The database-driven configuration enables real-time updates through PostgreSQL's NOTIFY/LISTEN mechanism \parencite{PostgreSQL181Documentation2025}. When an administrator modifies a configuration value through the user interface, the system broadcasts a notification to connected application instance. This server instance maintains an in-memory cache of configuration values that update immediately upon receiving the notifications. This architecture ensures configuration consistency while minimizing database queries during normal operation. The design addresses the operational phase requirements outlined in the RAGOps lifecycle \parencite{xuRAGOpsOperatingManaging2025}, where continuous monitoring and adjustment are necessary to maintain system effectiveness as underlying data and usage patterns evolve.

The configuration manager implements a callback system that triggers specific actions when critical settings change. For example, when the AI feature is enabled or disabled, registered callbacks handle necessary initialization or cleanup tasks. When embedding model configuration changes, the system automatically initiates reindexing of all documentation sources to ensure vector representations remain consistent with the active model. This reactive approach eliminates the need for manual intervention when configuration changes require downstream processing. The callback mechanism implements the adaptability principle from RAGOps \parencite{xuRAGOpsOperatingManaging2025}, where system modifications such as swapping embedding algorithms or switching LLM providers require automated propagation of changes through dependent components, ensuring consistency without manual coordination.

In Sovelia Core's Self Admin Tool, key configuration parameters include model and provider selection along with API keys and retrieval parameters (search criteria) to get the RAG system up and running. The caching layer ensures that frequently accessed configuration values, such as the AI enabled state, are retrieved from memory rather than requiring database queries for each check. Cache invalidation occurs automatically through the notification system, maintaining data freshness without polling overhead.
...
The embedding service manages resource utilization through queue-based processing with configurable concurrency limits. This prevents the service from overwhelming downstream systems during bulk documentation processing while maintaining reasonable throughput. This applied to prevent "port flooding" of the embedding provider when large document sets are processed, which could lead to rate limiting or service degradation. The queue-based architecture directly addresses the reliability quality attribute identified by \textcite{xuRAGOpsOperatingManaging2025}, where managing unpredictable factors such as bandwidth usage and operational costs requires load balancing, auto-scaling, and prioritization mechanisms to ensure consistent performance under varying workloads.

Database connection pooling ensures efficient use of database resources while preventing connection exhaustion during high query volumes. The connection pool parameters are tuned based on expected concurrent user load and available database capacity. This connection management strategy aligns with the resource usage monitoring recommended in the RAGOps framework \parencite{xuRAGOpsOperatingManaging2025}, where tracking infrastructure health and system resources forms a critical component of the operate and monitor phase in the query processing pipeline lifecycle.

For organizations using on-premises model deployment, GPU resource allocation becomes a critical consideration. The implementation supports configuring model serving instances separately from application servers, allowing deployment on specialized hardware with GPU acceleration. This deployment flexibility reflects the physical view considerations in RAG architecture \parencite{xuRAGOpsOperatingManaging2025}, where infrastructure decisions about hosting components on-premises versus cloud environments must accommodate varying organizational constraints around data governance, performance requirements, and available computational resources.
...
The chapter examined six major architectural components. The database schema (\autoref{sec:database-architecture}) employs PostgreSQL with pgvector extensions to provide unified storage for vector embeddings, conversation history, and configuration data, with specialized indexes supporting both similarity search and permission-filtered retrieval. The access control integration (\autoref{subsec:access-control-integration}) implements database-level permission filtering that respects Sovelia Core's existing security model, informed by recent research on permission-aware RAG systems \parencite{jeongPermissionAwareRAGIdentity2025, zhongHoneyBeeEfficientRolebased2025}. The provider abstraction layer (\autoref{subsec:provider-abstraction-layer}) addresses vendor lock-in risks through unified interfaces supporting multiple LLM and embedding providers, enabling deployment strategies ranging from fully on-premises (Ollama) to cloud-based services (OpenAI, Azure, Groq).

The documentation pipeline (\autoref{sec:documentation-pipeline-design}) implements a four-stage architecture—acquisition, chunking, embedding, and storage—designed for reliability and extensibility across heterogeneous document sources. The pipeline handles internal PLM documents through event-driven processing while managing external vendor documentation through scheduled updates, with format-specific processing maintaining semantic boundaries during chunking. Idempotent storage operations and version-aware deletion logic ensure consistency during document updates, addressing data management principles from the RAGOps framework \parencite{xuRAGOpsOperatingManaging2025}.

The retrieval mechanism (\autoref{sec:retrieval-mechanism}) combines vector similarity search with integrated permission checks, balancing retrieval quality, response time, and security requirements. Source tracking enables transparent provenance for all retrieved information, addressing trustworthiness concerns fundamental to enterprise AI adoption. Prompt engineering strategies (\autoref{sec:prompt-engineering}) implement structured templates that combine system instructions, conversation history, and retrieved context, with explicit anti-hallucination guidance and citation requirements reflecting regulatory transparency mandates \parencite{sillbergEUAIAct2025}.

The user interface integration (\autoref{sec:user-interface-integration}) presents a conversational chat interface supporting the diverse interaction patterns identified in research on generative AI usage \parencite{budiuTypesConversationsGenerative2023}, from concise pinpointing queries to extended exploring conversations. Message-level feedback mechanisms provide data for continuous improvement while source references enable seamless transition between AI-assisted discovery and detailed document review.

Operational considerations (\autoref{sec:operational-considerations}) address production deployment requirements through structured logging, dynamic configuration management using PostgreSQL's NOTIFY/LISTEN mechanism, and resource management strategies including queue-based processing and connection pooling. These operational design choices implement quality attributes identified in the RAGOps framework \parencite{xuRAGOpsOperatingManaging2025}: monitorability, observability, traceability, reliability, and adaptability.

The implementation reflects a synthesis of insights from reviewed case studies (\autoref{sec:case-studies-on-similar-integrations}) and Sovelia Core's specific requirements as an enterprise PLM system. The pragmatic approach prioritizes production-ready components and established patterns over experimental techniques, aligning with successful manufacturing documentation deployments \parencite{herediaalvaroAdvancedRetrievalaugmentedGeneration2025, shejutiExtendedAbstractEnhancing2025}. Simultaneously, the modular architecture and configurable parameters enable future enhancement with more advanced retrieval strategies, such as graph-based approaches \parencite{knollmeyerDocumentGraphRAGKnowledge2025, wanEmpoweringLLMsHybrid2025}, once the baseline system demonstrates value and achieves production stability. The choice of widely adopted open-source components (PostgreSQL, pgvector, LangChain) and support for multiple LLM providers reduces vendor lock-in risk while addressing the on-premises deployment and data governance requirements central to the research questions.
