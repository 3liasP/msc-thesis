\begin{aidisclaimer} * % ← Remove this asterisk, if you have not used AI in creating this thesis.

    I hereby declare, that the AI-based applications used in generating this work are as follows:

    \begin{center}
        \begin{tabularx}{\linewidth}{X|l}
            \toprule
            \textbf{Application}                                                     & \textbf{Version}                                               \\
            \midrule
            GitHub Copilot chat \parencite{githubAskingGitHubCopilota}               & Claude Sonnet 4.5 \parencite{anthropicIntroducingClaudeSonnet} \\
            GitHub Copilot code suggestions \parencite{githubGettingCodeSuggestions} & Grok Code Fast 1 \parencite{xaiGrokCodeFast}                   \\
            OpenAI API    \parencite{APIPlatformOpenAI}                              & gpt-5-nano \parencite{openaiGPT5NanoModel}                     \\
            Ollama local LLM  \parencite{Ollama}                                     & gpt-oss-20b \parencite{openaiGptoss20bModelOpenAI}             \\
            Ollama local embeddings \parencite{Ollama}                               & nomic-embed-text-v1.5 \parencite{nomicUnboxingNomicEmbed}      \\
            \bottomrule
        \end{tabularx}
    \end{center}

    \section*{Purpose of the use of AI}

    AI tools were used to enhance productivity and text quality. Claude Sonnet 4.5 assisted in improving clarity, grammar, and formatting of thesis text. Grok Code Fast 1 provided code suggestions during development. The models listed as gpt-oss-20b, gpt-5-nano, and nomic-embed-text were used exclusively within the RAG system implementation as research artifacts, not for writing this thesis.

    All research questions, design decisions, implementations, and conclusions are my own work.

\end{aidisclaimer}
